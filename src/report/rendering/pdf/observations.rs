...
impl QcObservationsReport {
    /// Renders this [QcObservationsReport] as PDF content.
    pub fn render_pdf(&self, opts: &QcRenderingOptions) -> genpdf::elements::LinearLayout {
            ...
            .sorted()
        {
                ...
                .get(&(descriptor.clone(), *constellation))
            {
                ...
                let chart_name = format!("{} {} Signal Power", descriptor, constellation);
...

                {
                        ...
                        data.storage.keys().map(|(sv, _)| sv).unique().enumerate()
                    {
                        for carrier in data.storage.keys().filter_map(|(sv_i, carrier)| {
                            ...
                            let sv_color = opts.color_map.color(nth_sv as f64 / cmap_len);

                            if let Some(data) = data.storage.get(&(*sv, *carrier)) {
